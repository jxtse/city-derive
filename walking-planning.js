...
function setupMapControls() {
    ...
    // 获取定位按钮
    safeAddEventListener('get-location', 'click', () => {
        requestUserLocation();
    });
...
function requestUserLocation() {
    console.log('📍 用户主动请求定位...');
    
    // 显示获取定位的提示
    showTemporaryMessage('📍 正在获取您的位置...', 'info');
    
    if (!navigator.geolocation) {
        showTemporaryMessage('❌ 您的浏览器不支持定位功能', 'error');
        return;
    }
    ...
    if (locationBtn) {
        locationBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 定位中...';
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // 恢复按钮状态
            if (locationBtn) {
                locationBtn.innerHTML = '<i class="fas fa-check"></i> 定位成功';
                locationBtn.style.background = 'linear-gradient(135deg, #28a745 0%, #20c997 100%)';
                
                setTimeout(() => {
                    locationBtn.innerHTML = '<i class="fas fa-location-arrow"></i> 获取定位';
        ...
        (error) => {
            ...
            // 恢复按钮状态
            if (locationBtn) {
                locationBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i> 定位失败';
...
function handleLocationSuccess(position) {
    ...
    console.log(`📍 用户位置: 经度${longitude}, 纬度${latitude}, 精度${accuracy}米`);
    
    // 将地图中心移动到用户位置
    if (map) {
        map.setCenter([longitude, latitude]);
        map.setZoom(16); // 放大到合适的级别
        
        // 添加用户位置标记
        const userIcon = createCustomIcon('start', 32);
        ...
        const infoContent = `
            <div style="padding: 12px; text-align: center;">
                <h4 style="margin: 0 0 8px 0; color: #2c3e50;">📍 您的当前位置</h4>
                <p style="margin: 0; color: #7f8c8d; font-size: 12px;">
                    经度: ${longitude.toFixed(6)}<br>
                    纬度: ${latitude.toFixed(6)}<br>
                    精度: ${Math.round(accuracy)}米
                </p>
                <button onclick="fillLocationInForm(${longitude}, ${latitude})" 
                        style="margin-top: 8px; background: #28a745; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 11px;">
                    设为起点
                </button>
            </div>
        `;
    ...
    // 显示成功消息
    showTemporaryMessage(`✅ 定位成功！精度: ${Math.round(accuracy)}米`, 'success');
...
function handleLocationError(error) {
    ...
    switch(error.code) {
        ...
        case error.POSITION_UNAVAILABLE:
            ...
            break;
        case error.TIMEOUT:
            errorMessage = '❌ 定位请求超时。请重试或检查网络连接。';
            break;
        default:
...
async function reverseGeocode(longitude, latitude) {
    try {
        ...
        if (data.status === '1' && data.regeocode) {
            ...
            console.log('📍 用户位置地址:', address);
            
            // 更新信息窗体内容
            showTemporaryMessage(`📍 您的位置: ${address}`, 'info');
...
// 将位置信息填充到表单中
window.fillLocationInForm = function(longitude, latitude) {
    // 获取地址并填充
    reverseGeocode(longitude, latitude);
    
    showTemporaryMessage('✅ 位置已设为起点', 'success');
...
function showLocationGuide() {
    ...
    guideModal.innerHTML = `
        <div style="background: white; padding: 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center;">
            <h3 style="color: #2c3e50; margin-bottom: 20px;">📍 如何开启定位权限？</h3>
            <div style="text-align: left; margin-bottom: 20px; color: #333; line-height: 1.6;">
...
function initMap() {
    try {
        ...
        map.on('complete', function() {
            ...
            // 地图加载完成后，提示用户可以获取定位
            setTimeout(() => {
                showTemporaryMessage('💡 点击右上角的"获取定位"按钮来定位您的位置', 'info');
            }, 2000);
